...
# # # # # # # # # # # # # # # # # # # # # # # # # #
        A="*Reading in .wav*"
        if os.path.exists("compressed.wav"):
            os.remove("compressed.wav")
            print('check track')
            exit(2)

        # Read in the sample rate and audio samples/amplitudes of the wav file
        if os.path.exists("original.wav"):
            sample_rate, audio = scipy.io.wavfile.read("original.wav")
        else:
            print('Your file is missing, please try again.')
            exit(2)
